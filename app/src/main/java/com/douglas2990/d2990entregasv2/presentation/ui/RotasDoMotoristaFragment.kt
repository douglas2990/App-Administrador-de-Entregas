...
class RotasDoMotoristaFragment : Fragment() {
    ...
    private fun setupRecyclerView() {
        // 🚀 Atualizado para o novo construtor do RotaAdapter com os 3 parâmetros limpos
        rotaAdapter = RotaAdapter(
            ...
            onEditarClick = { rota ->
                if (!isArquivado) {
                    ...
                    Toast.makeText(context, "Não é possível editar rotas do histórico", Toast.LENGTH_SHORT).show()
                }
            },
            onDeletarClick = { rota ->
                if (!isArquivado) {
            ...
            when (status) {
                ...
                is UIstatus.Sucesso -> {
                    ...
                    if (lista.isEmpty()) {
                        ...
                        rotaAdapter.submitList(emptyList()) // Garante lista limpa visualmente
                    } else {
        ...
        viewModel.statusExclusao.observe(viewLifecycleOwner) { status ->
            when (status) {
                ...
                is UIstatus.Sucesso -> {
                    ...
                    // Atualiza a lista baseado no filtro ativo no momento
                    motorista?.let { mot ->
                        when {
                            dataSelecionada != -1L -> viewModel.listarPorDataEMotorista(mot.id, dataSelecionada)
                            else -> viewModel.listarRotasMotorista(mot.id)
                        }
                    }
                }
                is UIstatus.Erro -> {
    ...
    private fun abrirTelaEdicao(rota: Rota) {
        ...
        // Caso use o Navigation Component tradicional por ID:
        findNavController().navigate(R.id.action_rotasDoMotoristaFragment_to_fragmentEditarRota, bundle)
    }

    private fun exibirDialogoConfirmacao(rota: Rota) {
        AlertDialog.Builder(requireContext())
            .setTitle("Excluir Rota")
            .setMessage("Deseja realmente excluir a rota da OS ${rota.os}? Ela sumirá do aplicativo do motorista.")
