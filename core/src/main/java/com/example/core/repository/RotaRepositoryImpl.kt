...
class RotaRepositoryImpl @Inject constructor(
    ...
    override fun listarPorMotoristaRealTime(
        ...
        // 1. Avisamos a UI que estamos carregando
        onResult(UIstatus.Carregando)

        // 2. Criamos o "ouvinte" (SnapshotListener)
        colecaoRotas
            .whereEqualTo("idMotorista", idMotorista)
            .orderBy("dataCriacao", Query.Direction.DESCENDING)
            .addSnapshotListener { snapshot, erro ->

                // Se houver erro (ex: falta de internet ou erro de índice)
                if (erro != null) {
                    // Tentativa automática sem ordenação (caso o índice falhe)
                    colecaoRotas.whereEqualTo("idMotorista", idMotorista)
                        .addSnapshotListener { snapshotSemOrdem, erro2 ->
                            if (erro2 != null) {
                                onResult(UIstatus.Erro("Erro ao sincronizar: ${erro2.message}"))
                            } else {
                                val lista = snapshotSemOrdem?.toObjects(Rota::class.java) ?: emptyList()
                ...
                // Sucesso: Toda vez que o Guilherme mudar algo no Admin, esse bloco executa sozinho!
